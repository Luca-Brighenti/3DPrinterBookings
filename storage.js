...
const allowedExtensions = new Set(['.3mf', '.stl', '.obj', '.step', '.stp', '.gcode']);
...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    if (!allowedExtensions.has(ext)) {
      return cb(new Error('File type not allowed. Upload a .3mf, .stl, .obj, .step, .stp, or .gcode file.'));
    }
    cb(null, true);
  }
});

function createUploadMiddleware(fieldName = 'file') {
