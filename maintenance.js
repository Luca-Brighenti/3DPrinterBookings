...
const fileRetentionDays = Math.max(1, Number(process.env.FILE_RETENTION_DAYS || 1));
...
async function runLocalOrphanCleanup() {
  ...
  const { rows } = await pool.query(
    `SELECT file_path FROM bookings
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (Date.now() - stats.mtimeMs < 30 * 60 * 1000) continue;
      await fs.promises.unlink(fullPath);
      removed += 1;
    } catch { /* file disappeared between stat and unlink */ }
  }

  const tmpDir = path.join(uploadsDir, 'tmp');
  const tmpEntries = await fs.promises.readdir(tmpDir, { withFileTypes: true }).catch(() => []);
